...
def Isanycolumnzero(_energy):
    bool_zero = False
    for i in range(0, np.shape(_energy)[1] - 1):
        _energy_num = np.expand_dims(_energy[:, i], axis=1)
        temp_ave = np.mean(_energy_num)
        _energy_num2 = _energy_num-temp_ave
        if np.any(_energy_num2>=0.000000001):
            continue
        else:
            bool_zero = True
            break
