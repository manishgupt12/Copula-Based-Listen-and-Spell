...
from model.cbls_model import Listener,Speller
...
for epoch in range(num_epochs):
    ...
    print("Yo! relax ", speller.count_original,speller.count_copula)
    ...
    for batch_index,(batch_data,batch_label) in enumerate(train_set):
        ...
        if (batch_index+1) % verbose_step == 0:
            print(training_msg.format(epoch+1,batch_index+1,tr_loss/(batch_index+1),sum(tr_ler)/len(tr_ler)),end='\r',flush=True)
    ...
    # Logger
    print(epoch_end_msg.format(epoch+1,tr_loss/(batch_index+1),sum(tr_ler)/len(tr_ler),
                               tt_loss/len(test_set),sum(tt_ler)/len(tt_ler),training_time),flush=True)
    print(epoch_end_msg.format(epoch+1,tr_loss/(batch_index+1),sum(tr_ler)/len(tr_ler),
                               tt_loss/len(test_set),sum(tt_ler)/len(tt_ler),training_time),flush=True,file=traing_log)
